...
class WebUIGeneralContext extends RawMinkContext implements Context {
	 ...
	 * @param string $errorMessage which number of multiple emails to read (first email is 1)
	 * @param int $no
	 */
	public function followLinkFromEmail($emailAddress, $regexSearch, $errorMessage, $no=1) {
		$content = EmailHelper::getBodyOfEmail(
			EmailHelper::getMailhogUrl(), $emailAddress, $no
		);
		$matches = [];
	...
	/**
	 * @Then the user should be redirected to general error webUI page with the title :title
	 *
	 * @param string $title
	 *
	 * @return void
